...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtFechaEntrada.setBounds(68, 161, 289, 35);
                //No se puede ingresar fechas anteriores a la fecha actual
                txtFechaEntrada.getJCalendar().setMinSelectableDate(new Date());
		...
		txtFechaSalida.setBounds(68, 246, 289, 35);
                //No se puede ingresar fechas anteriores a la fecha actual
                txtFechaSalida.getJCalendar().setMinSelectableDate(new Date());
		txtFechaSalida.getCalendarButton().setBounds(267, 1, 21, 31);
		txtFechaSalida.setBackground(Color.WHITE);
		txtFechaSalida.setFont(new Font("Roboto", Font.PLAIN, 18));
                //Configuración de la fecha de salida aplicando concurrencia
		txtFechaSalida.addPropertyChangeListener(evt -> {
				if(txtFechaEntrada.getDate()!=null && txtFechaSalida.getDate()!=null) {   
                                    // Creamos un SwingWorker para cálculo en segundo plano
                                    SwingWorker<Integer, Void> worker = new SwingWorker<>() {
                                        @Override
                                        protected Integer doInBackground() throws Exception {
                                            return valorReserva(); // Metodo que calcula días * precio
                                        }
                                        @Override
                                        protected void done() {
                                            try {
                                                int valor = get(); // Se obtiene el resultado
                                                if (valor <= 0) { // Misma fecha o salida antes de entrada
                                    ...
                                    worker.execute(); // ejecuta el hilo en segundo plano
				}	
		});
